...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 4  # 4 corners visited
        ...
        for action in [
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextx, nexty) in self.corners and (
                    nextx,
                    nexty,
                ) not in corner_visited:
                    nextState = ((nextx, nexty), corner_visited + [(nextx, nexty)])
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        if corner not in corner_visited:
            unvisited.append(corner)
    # Find sum of the shortest distances between the unvisited corners
    heuristic = 0
    while unvisited:
        distance, corner = min(
            [(util.manhattanDistance(pos, corner), corner) for corner in unvisited]
        )
        heuristic += distance
        # update cur pos to calculate shortest distance from cur corner to next corner
        pos = corner
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        "*** YOUR CODE HERE ***"
        return self.food[x][y]  # check if food is available at given state
